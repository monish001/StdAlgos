...
*/
int _findMaxSumFromStart(const vector<int> &A, const int start, const int end){
...
int _findMaxSumSubarray(const vector<int> &A, const int start, const int end){
    ...
    // Option 3:
    int sumRightSide = _findMaxSumFromStart(A, middle+1, end),
    sumLeftSide = _findMaxSumFromStart(A, middle-1, start);
    int option3 = sumRightSide + sumLeftSide + A[middle];

    cout<<"opt 1: "<<option1<<endl<<"opt 2:"<<option2<<endl<<"opt 3:"<<option3<<endl;
